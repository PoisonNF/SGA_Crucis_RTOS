...
void UserLogic_Code(void)
{
	...
	//创建线程
	thread1 = rt_thread_create("Motioncontrol",Motioncontrol,NULL,512,3,20);
	if(RT_NULL != thread1)
	{
	...
	thread2 = rt_thread_create("JY901S",JY901S_thread,NULL,512,4,20);
	if(RT_NULL != thread2)
	{
	...
	thread3 = rt_thread_create("PS2",PS2_thread,NULL,512,3,20);
	if(RT_NULL != thread3)
	{
	...
	thread4 = rt_thread_create("Huba511",Huba511_thread,NULL,512,4,20);
	if(RT_NULL != thread4)
	{
	...
	thread5 = rt_thread_create("Rm3100",Rm3100_thread,NULL,512,5,20);
	if(RT_NULL != thread5)
	{
	...
	thread6 = rt_thread_create("Test",Test_thread,NULL,512,6,20);
	if(RT_NULL != thread6)
	{
